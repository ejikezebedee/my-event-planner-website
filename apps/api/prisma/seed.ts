...
async function main() {
  ...
  const planner = await prisma.user.create({
    data: {
      email: "planner@eventplanner.dev",
      name: "Paula Planner",
      passwordHash,
      emailVerifiedAt: new Date(),
    },
  ...
  const budgetItems = await prisma.$transaction([
    ...
    prisma.budgetItem.create({
      data: {
        eventId: event.id,
        categoryId: cat("Catering").id,
        name: "Catering 80 guests",
    ...
    prisma.budgetItem.create({
      data: {
        eventId: event.id,
        categoryId: cat("Photography").id,
        name: "Photographer",
        plannedAmount: 220_000,
        priority: "medium",
        createdById: demo.id,
      },
    }),
  ...
  const [venueVendor, cateringVendor, photoVendor] = await Promise.all([
    prisma.vendor.create({
      data: {
        workspaceId: workspace.id,
        businessName: "Lakeside Manor GmbH",
        serviceCategory: "Venue",
        email: "events@lakeside.example",
        city: "Berlin",
        createdById: demo.id,
      },
    }),
    prisma.vendor.create({
      data: {
        ...
        email: "hello@finecatering.example",
        createdById: demo.id,
      },
    }),
    prisma.vendor.create({
    ...
    data: [
      {
        ...
        status: "contracted",
        paymentDueDate: inDays(90),
      },
      {
        eventId: event.id,
        vendorId: cateringVendor.id,
        quotedAmount: 410_000,
        agreedAmount: 400_000,
        status: "deposit_paid",
        paymentDueDate: inDays(60),
      },
      {
        eventId: event.id,
        vendorId: photoVendor.id,
        quotedAmount: 220_000,
  ...
  const venue = await prisma.expense.create({
    data: {
      ...
      vendorId: venueVendor.id,
      expenseDate: agoDays(30),
      subtotal: 420_169,
      taxAmount: 79_831,
      totalAmount: 500_000,
      dueDate: agoDays(10),
      status: "paid",
      invoiceNumber: "LM-2025-041",
      createdById: demo.id,
    },
  });
  const catering = await prisma.expense.create({
    data: {
      eventId: event.id,
      title: "Catering — first instalment",
      categoryId: cat("Catering").id,
      budgetItemId: cateringItem.id,
      vendorId: cateringVendor.id,
      expenseDate: agoDays(20),
      subtotal: 336_134,
      taxAmount: 63_866,
      totalAmount: 400_000,
      dueDate: inDays(30),
  ...
  const photo = await prisma.expense.create({
    data: {
      ...
      categoryId: cat("Photography").id,
      budgetItemId: photoItem.id,
      vendorId: photoVendor.id,
      expenseDate: agoDays(12),
      subtotal: 168_067,
      taxAmount: 31_933,
      totalAmount: 200_000,
      dueDate: inDays(45),
  ...
  await prisma.expense.createMany({
    data: [
      {
        eventId: event.id,
        title: "Bridal flowers & table decor",
        categoryId: cat("Decoration").id,
        budgetItemId: flowersItem.id,
        expenseDate: agoDays(5),
        subtotal: 84_034,
        taxAmount: 15_966,
        totalAmount: 100_000,
        dueDate: agoDays(2),
      ...
      },
      {
        ...
        title: "Live band",
        categoryId: cat("Entertainment").id,
        budgetItemId: bandItem.id,
        expenseDate: agoDays(3),
        subtotal: 42_017,
        taxAmount: 7_983,
        totalAmount: 50_000,
        dueDate: inDays(80),
  ...
  await prisma.payment.createMany({
    data: [
      ...
      // Partial refund: the photographer returned the overcharged travel fee.
      {
        ...
        vendorId: photoVendor.id,
        type: "refund",
        refundOfPaymentId: photoPayment.id,
        amount: 10_000,
        paidAt: agoDays(2),
        method: "credit_card",
        reference: "RF-101",
        notes: "Travel fee returned",
        createdById: demo.id,
      },
    ],
  });
  ...
  const t1 = await prisma.task.create({
    data: {
      ...
      status: "completed",
      priority: "critical",
      completedAt: agoDays(25),
      completedById: demo.id,
      dueAt: agoDays(25),
      createdById: demo.id,
    },
  });
