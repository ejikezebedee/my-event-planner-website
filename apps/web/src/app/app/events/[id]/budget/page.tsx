      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex justify-between text-sm">
            <span>
              Planned{" "}
              <MoneyText minor={totals.planned} currency={currency} className="font-medium" /> ·
              Actual <MoneyText minor={totals.actual} currency={currency} className="font-medium" />{" "}
              · Budget{" "}
              <MoneyText
                minor={event.data.budgetAmount}
                currency={currency}
          ...
          <Progress
            value={
              event.data.budgetAmount > 0
                ? Math.min(100, (totals.actual / event.data.budgetAmount) * 100)
                : 0
            }
          />
          {totals.actual > event.data.budgetAmount && event.data.budgetAmount > 0 && (
            <p className="text-sm font-medium text-destructive">
              Over budget by{" "}
              <MoneyText minor={totals.actual - event.data.budgetAmount} currency={currency} />
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <Card key={category.id}>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <span>
                Planned <MoneyText minor={category.planned} currency={currency} />
              </span>
              <span>
                Actual{" "}
                <MoneyText
                  minor={category.actual}
                  currency={currency}
                  className={
                    category.actual > category.planned && category.planned > 0
                      ? "text-destructive"
                      : ""
                  }
                />
              ...
              {category.utilisationPercent != null && (
                <span
                  className={
                    category.utilisationPercent > 100 ? "font-medium text-destructive" : ""
                  }
                >
                  {category.utilisationPercent}% used
                </span>
              )}
              <Button size="sm" variant="outline" onClick={() => setItemDialog(category.id)}>
                  ...
                  {category.items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right">
                        <MoneyText minor={item.plannedAmount} currency={currency} />
                      </TableCell>
                      <TableCell className="text-right">
                        <MoneyText minor={item.actual ?? 0} currency={currency} />
                      </TableCell>
                      <TableCell
                        className={`text-right ${(item.variance ?? 0) < 0 ? "text-destructive" : "text-emerald-600"}`}
                      >
                        {(item.variance ?? 0) < 0 ? "−" : "+"}
                        <MoneyText minor={Math.abs(item.variance ?? 0)} currency={currency} />
                      </TableCell>
                      <TableCell
                        className={`text-right ${(item.utilisationPercent ?? 0) > 100 ? "font-medium text-destructive" : "text-muted-foreground"}`}
                      >
                      ...
                      <TableCell>
                        <Button
                          size="icon"
                          variant="ghost"
                          aria-label={`Delete budget item ${item.name}`}
