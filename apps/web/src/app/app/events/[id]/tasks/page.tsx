...
export default function TasksPage() {
  ...
  const [form, setForm] = useState({ title: "", priority: "medium", dueAt: "", dependencyIds: [] as number[] });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add task</Button>}
      />
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={overdueOnly} onChange={(ev) => setOverdueOnly(ev.target.checked)} />
          Overdue only
        </label>
      </div>
      <div className="rounded-lg border">
        <Table>
          ...
          <TableBody>
            {visible.length === 0 && (
              <TableRow><TableCell colSpan={6} className="py-8 text-center text-muted-foreground">No tasks match.</TableCell></TableRow>
            )}
            {visible.map((t) => {
              return (
                <TableRow key={t.id}>
                  <TableCell className="font-medium">
                    ...
                    {t.blockedBy.length > 0 && t.status !== "completed" && (
                      <span className="ml-2 text-xs text-amber-600" title={`Blocked by: ${t.blockedBy.map((b) => b.title).join(", ")}`}>
                        (blocked by {t.blockedBy.map((b) => b.title).join(", ")})
                      </span>
                    )}
                  </TableCell>
                  <TableCell>{labelize(t.priority)}</TableCell>
                  <TableCell className={t.overdue ? "font-medium text-destructive" : ""}>{formatDate(t.dueAt)}</TableCell>
                  <TableCell className="text-muted-foreground">
                    {t.dependencyIds.map((id) => byId.get(id)?.title ?? `#${id}`).join(", ") || "—"}
                  </TableCell>
                  <TableCell>
                    <Select value={t.status} onValueChange={(v) => setStatus.mutate({ task: t, status: v })}>
                      <SelectTrigger className="h-8 w-36"><SelectValue /></SelectTrigger>
                      <SelectContent>{TASK_STATUSES.map((s) => <SelectItem key={s} value={s}>{labelize(s)}</SelectItem>)}</SelectContent>
                    </Select>
                  </TableCell>
                  <TableCell>
                    <Button size="icon" variant="ghost" aria-label={`Delete task ${t.title}`} onClick={() => remove.mutate(t.id)}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              );
            })}
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); create.mutate(); }}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="max-h-40 space-y-1 overflow-y-auto rounded-md border p-2">
                {tasks.data.filter((t) => t.status !== "completed").map((t) => (
