...
import {
  Badge, Button, Dialog, DialogContent, DialogHeader, DialogTitle, Input, Label, Select,
...
export default function TimelinePage() {
  ...
  return (
    <div>
      <PageHeader title="Timeline" actions={<Button onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add entry</Button>} />
          ...
          {entries.data.map((entry) => {
            ...
            return (
              <li key={String(entry.id)} className="relative">
                <span className={`absolute -left-[31px] top-1 h-3 w-3 rounded-full ${isAuto ? "bg-muted-foreground/50" : "bg-primary"}`} />
                <div className="flex items-start justify-between">
                  ...
                  {!isAuto && (
                    <Button size="icon" variant="ghost" aria-label={`Delete ${entry.title}`} onClick={() => remove.mutate(Number(entry.id))}><Trash2 className="h-4 w-4" /></Button>
                  )}
                </div>
              </li>
            );
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); create.mutate(); }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}>
                  ...
                  <SelectContent>{TIMELINE_TYPES.map((t) => <SelectItem key={t} value={t}>{labelize(t)}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label>Start</Label>
            ...
            <Button type="submit" className="w-full" disabled={create.isPending || !form.startAt}>Add entry</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
