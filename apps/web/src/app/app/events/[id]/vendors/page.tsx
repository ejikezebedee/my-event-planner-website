...
export default function VendorsPage() {
  ...
  const filteredVendors = useQuery({
    ...
    queryFn: () =>
      api.get<Vendor[]>(
        `/vendors?workspaceId=${current!.id}${vendorSearch ? `&q=${encodeURIComponent(vendorSearch)}` : ""}`,
      ),
  ...
  const summary = useQuery({
    ...
    queryFn: () =>
      api.get<
        Vendor & {
          summary: { events: number; quoted: number; agreed: number; paid: number };
          links: {
            id: number;
            status: string;
  ...
  return (
    <div>
            ...
            {links.data.map((link) => (
              <TableRow key={link.id}>
                ...
                <TableCell className="text-muted-foreground">
                  {link.vendor.serviceCategory ?? "—"}
                </TableCell>
                <TableCell className="text-right">
                  {link.quotedAmount != null ? (
                    <MoneyText minor={link.quotedAmount} currency={currency} />
                  ) : (
                    "—"
                  )}
                </TableCell>
                <TableCell className="text-right">
                  {link.agreedAmount != null ? (
                    <MoneyText minor={link.agreedAmount} currency={currency} />
                  ) : (
                    "—"
                  )}
                </TableCell>
                <TableCell className="text-right">
                  <MoneyText minor={link.paid} currency={currency} />
                </TableCell>
                <TableCell>
                  <Select
                    value={link.status}
                    onValueChange={(v) => setStatus.mutate({ link, status: v })}
                  >
                ...
                <TableCell>
                  <Button
                    size="icon"
                    variant="ghost"
                    aria-label={`Remove ${link.vendor.businessName}`}
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setAssignForm({ ...assignForm, vendorId: v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select vendor" />
                </SelectTrigger>
                <SelectContent>
                  {(filteredVendors.data ?? workspaceVendors.data)?.map((v) => (
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setAssignForm({ ...assignForm, status: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <Button
              type="submit"
              className="w-full"
              disabled={assign.isPending || !assignForm.vendorId}
            >
              Assign
            </Button>
          </form>
      ...
      <Dialog open={summaryFor != null} onOpenChange={(o) => !o && setSummaryFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{summary.data?.businessName ?? "Vendor"}</DialogTitle>
          </DialogHeader>
          {!summary.data ? (
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <MoneyText
                    minor={summary.data.summary.quoted}
                    currency={currency}
                ...
                <div>
                  ...
                  <MoneyText
                    minor={summary.data.summary.agreed}
                    currency={currency}
                    className="font-medium"
                  />
                </div>
                <div>
                  ...
                  <MoneyText
                    minor={summary.data.summary.paid}
                    currency={currency}
                    className="font-medium"
                  />
                </div>
              ...
              <div>
                <p className="mb-1 text-xs text-muted-foreground">
                  Linked events ({summary.data.summary.events})
                </p>
                  ...
                  {summary.data.links.map((l) => (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-2 text-muted-foreground">
                        {l.rating != null && (
                          <span title="Internal rating — not publicly visible">★ {l.rating}/5</span>
                        )}
                        {l.agreedAmount != null && (
                          <MoneyText minor={l.agreedAmount} currency={currency} />
                        )}
              ...
              <p className="text-xs text-muted-foreground">
                Internal ratings are never publicly visible.
              </p>
            </div>
          )}
        </DialogContent>
